...
class Load_Dataset(Dataset):
    ...
    def __init__(self, dataset, config, training_mode):
        ...
        labels = [d['summary']['QD'][1:]/nominal_capacity for d in dataset]
        ...
        for i in range(2):
            ...
            cycles = sorted(cell_data.keys(), key=lambda x: int(x))[1:]
            records.append(np.asarray([[scaler.fit_transform(cell_data[str(c)][k][start:head].reshape(-1, 1)).flatten() for k in ks] for c in cycles], dtype=np.float32))
