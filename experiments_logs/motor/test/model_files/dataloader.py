...
class Load_Dataset(Dataset):
    ...
    def __init__(self, dataset, config, training_mode):
        ...
        for i in range(len(dataset)):
            cell_data = dataset[i]
            cycles = sorted(cell_data.keys(), key=lambda x: int(x))[1:]
            records.append(np.asarray([[cell_data[str(c)][k][:head] for k in ks] for c in cycles], dtype=np.float32))
    ...
    def __getitem__(self, index):
        ...
        if self.training_mode == "self_supervised" or self.training_mode == "SupCon":
            return self.x_data[i][si], si + 1, self.aug1[i][si], self.aug2[i][si]
        else:
            return self.x_data[i][si], si + 1, self.x_data[i][si], self.x_data[i][si]
