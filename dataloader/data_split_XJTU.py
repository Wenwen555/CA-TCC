...
class Load_Dataset():
    ...
    def train_val_test_split(data):
        # we simulate article CL in battery, pick c-4, c-7 as test bat
        # because batch-1 only stores 8 bat.
        # test_bat_idx = ['2C_battery-4', '2C_battery-8']  # Batch-1
        # test_bat_idx = ['3C_battery-4', '3C_battery-8', '3C_battery-14']  # Batch-2
        test_bat_idx = ['R2.5_battery-4', 'R2.5_battery-8']  # Batch-3
        train_val = []
        test_bat = []
        for bat in data:
            if bat not in test_bat_idx:
                train_val.append(data[bat])
        # 此处不需要split labels，因为labels是存在data内部的
        random.shuffle(train_val)
        train_len = int(len(train_val) * 0.90)
        for bat in test_bat_idx:
            test_bat.append(data[bat])
...
save_path = os.path.join(parent_path, 'data/XJTU/cell_XJTU_batch3_prepocess_data')
filepath = os.path.join(parent_path, 'data/XJTU/XJTU_batch3_prepocess.pkl')
