...
class TC(nn.Module):
    ...
    def forward(self, features_aug1, features_aug2):
        ...
        encode_samples = torch.empty((self.window_len, batch, self.num_channels)).float().to(self.device)

        for i in np.arange(1, self.window_len + 1):
            encode_samples[i - 1] = z_aug1[:, t_samples + i, :].view(batch, self.num_channels)
        # forward_seq = z_aug1[:, :t_samples + 1, :]

        pred = torch.empty((self.window_len, batch, self.num_channels)).float().to(self.device)
        for i in range(1, self.window_len+1):
            p = t_samples + i
            e = p + 1 + self.timestep
            forward_seq = torch.concatenate((z_aug1[:, :p, :], z_aug1[:, p + 1:e, :]), dim=1)
        ...
        for i in np.arange(0, self.window_len):
            total = torch.mm(encode_samples[i], torch.transpose(pred[i], 0, 1))
            nce += torch.sum(torch.diag(self.lsoftmax(total)))
        nce /= -1. * batch * self.window_len
