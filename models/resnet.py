...
class ResNet(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv1d(4, self.inplanes, kernel_size=7, stride=1, padding=3, bias=False)
        # MIT
        # self.conv1 = nn.Conv1d(5, self.inplanes, kernel_size=7, stride=1, padding=3, bias=False)
        ...
        # modified resnet < 50
        self.layer1 = self._make_layer(block, 8 * 4, layers[0])
        self.layer2 = self._make_layer(block, 16 * 4, layers[1], stride=2)
        self.layer3 = self._make_layer(block, 32 * 4, layers[2], stride=2)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        flatten = torch.flatten(feature, 1)
